...
```
El puerto 3000 debe estar en **Público** (Port Visibility → Public) para que la app pueda llamar al servicio; **al terminar la prueba debe volver a Privado o eliminarse el Codespace**. Además, la app corre en otro origen (puerto 8081), por lo que el servicio debe permitirlo mediante CORS: el `docker-compose.yml` define `ORIGENES_PERMITIDOS` (localhost y, en Codespaces, la URL del puerto 8081 armada con `CODESPACE_NAME`). En la app: pegar la URL del puerto 3000, pegar el **token** (solo el valor de `access_token`, sin la palabra «Bearer»: la app la antepone), escribir un texto, pulsar «Cifrar» y luego «Descifrar». Si el texto pasa de 190 bytes el servicio responde 400.
